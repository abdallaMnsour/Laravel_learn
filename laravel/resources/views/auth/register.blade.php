<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-4">
            ...
            <x-text-input id="car" class="block mt-1 w-full" type="select" name="car" :value="old('car')" required autocomplete="car">
                <option value="{{ old('car') }}">{{ $cars->find(old('car'))->name ?? '' }}</option>
                @foreach ($cars as $car)
